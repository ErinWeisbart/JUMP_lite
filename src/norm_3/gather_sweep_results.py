...
def generate_codec_delta_from_raw_plot(pdf, output_dir: Path, model_colors: dict,
    ...
    # --- Plot: 2 rows (absolute delta, percentage delta) x n_metrics cols ---
    n_rows = 2
    row_height = max(5, 1.2 * n_families)
    fig, all_axes = plt.subplots(n_rows, n_metrics,
                                 figsize=(7 * n_metrics, row_height * n_rows))
    if n_metrics == 1:
        all_axes = all_axes.reshape(n_rows, 1)

    row_configs = [
        # (row_idx, value_col, xlabel_template, title_suffix)
    ...
    for row_idx, value_col, xlabel_tpl, title_sfx in row_configs:
        ...
        for col_idx, (metric_title, metric_col) in enumerate(metrics):
            ...
            for fam, cl, m, _ in codec_entries:
                ...
                if len(vals) > 0:
                    # Individual config deltas as jittered dots
                    y_jitter = np.random.normal(ypos, 0.12, len(vals))
                    ax.scatter(vals, y_jitter, c=[color], s=20, alpha=0.4,
                               edgecolors="white", linewidths=0.2, zorder=3)
                    # Mean as a larger marker
                    mean_val = np.mean(vals)
                    ax.scatter(mean_val, ypos, c=[color], s=120, alpha=1.0,
                               edgecolors="black", linewidths=0.8, marker="D", zorder=5)
                    # Horizontal line from 0 to mean
                    ax.plot([0, mean_val], [ypos, ypos], color=color, linewidth=1.5,
                            alpha=0.7, zorder=2)
                    # Annotate mean % value in the percentage row
                    if is_pct_row:
