...
The only annotation file committed to the repo is `metadata/motive_splits.parquet`
...
| 8 | MOTIVE splits parquet | 2b (`metadata/motive_splits.parquet`) AND `evaluate_motive.py` (compound allowlist) | ✅ **Committed** at `metadata/motive_splits.parquet` (94 KB; 26,450 rows; not a train/test partition — all rows labelled `"test"`, used as a JCP2022 allowlist of MOTIVE benchmark perturbations). Source: canonicalised from the MOTIVE publication supplement. |
