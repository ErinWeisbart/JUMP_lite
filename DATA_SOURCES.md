...
| CellProfiler `profiles.parquet` | External CellProfiler pipeline run | **TODO** — see §2 (CellProfiler) |

All annotation files under `metadata/*.parquet` (incl. `motive_*` and the master
`metadata_dataset_filtered_4reps.parquet`) are **committed**. A reproducer who
trusts those does not need anything in §2 below — the references are kept for
provenance only.
...
| `annotations_compound_gene.parquet` (MOTIVE) | `motive_splits.parquet` (canonicalised copy of the input) |
...
| MOTIVE splits parquet (`--motive-splits-path`, published with the MOTIVE paper) | |
...
| 8 | MOTIVE splits parquet | 2b (copied to `metadata/motive_splits.parquet`) | **TODO** — supplementary of the MOTIVE publication; manual download |
| 9 | CellProfiler `profiles.parquet` | §1 (eval) **AND** 2a | **TODO** — pipeline definition / canonical version. This is the only §2 entry that is also blocking for `produce-paper` itself. |
