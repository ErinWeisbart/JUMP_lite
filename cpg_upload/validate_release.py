...
EXPECTED_WELL_COUNT = 213_881
EXPECTED_PLATE_COUNT = 557
...
def main() -> int:
    ...
    if not canonical_path.is_dir():
        ...
        canonical_keys = directory_keys(canonical_path)
    ...
    for codec in IMAGE_CODECS:
        ...
        require(
            ...
            f"Image keys differ for {codec}: missing={len(missing):,}, extra={len(extra):,}",
        )
    ...
    for model, codecs in sorted(EXPECTED_PROFILE_VARIANTS.items()):
        for codec in sorted(codecs):
            ...
            require(
                not missing and not extra,
                f"Per-site Parquet keys differ for {model}/{codec}: "
                f"missing={len(missing):,}, extra={len(extra):,}",
            )
    ...
    if all(path.is_file() for path in parquet_paths):
        ...
        index_rows, index_distinct = con.execute(
            f"SELECT count(*), count(DISTINCT Metadata_Site_Key) "
            f"FROM read_parquet('{site_index}')"
        ).fetchone()
        index_keys = {
            row[0]
        ...
        well_rows, distinct_wells = con.execute(
            f"SELECT count(*), count(DISTINCT Metadata_Well_Key) "
            f"FROM read_parquet('{perturbation}')"
        ).fetchone()
        require(
