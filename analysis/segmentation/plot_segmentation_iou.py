...
cell_iou = load_segmentation_data('output/segmentation_comparison/segmentation_comparison_segment_cell_summary.csv')
nuclei_iou = load_segmentation_data('output/segmentation_comparison/segmentation_comparison_segment_nuclei_summary.csv')
...
def create_iou_plot(cell_iou_values, nuclei_iou_values, output_path):
    ...
    ax.set_xlabel('File Size (% of ZSTD)', fontsize=16, fontweight='bold')
    ...
    line_cell = ax.plot(sizes_sorted, cell_ious_sorted, 's-', color=color_cell, linewidth=2.5,
                   markersize=14, label='Cell Segmentation', markeredgewidth=2,
                   markeredgecolor='white')

    # Plot Nuclei IoU
    color_nuclei = 'tab:blue'
    line_nuclei = ax.plot(sizes_sorted, nuclei_ious_sorted, 'o-', color=color_nuclei, linewidth=2.5,
                   markersize=14, label='Nuclei Segmentation', markeredgewidth=2,
                   markeredgecolor='white')
    ...
    for i, (x, y) in enumerate(zip(sizes_sorted, cell_ious_sorted)):
        ax.annotate(f'{y:.3f}', (x, y), textcoords="offset points",
                   xytext=(0, -20), ha='center', fontsize=12, color=color_cell,
    ...
    for i, (x, y) in enumerate(zip(sizes_sorted, nuclei_ious_sorted)):
        ax.annotate(f'{y:.3f}', (x, y), textcoords="offset points",
                   xytext=(0, 18), ha='center', fontsize=12, color=color_nuclei,
                   fontweight='bold', bbox=dict(boxstyle='round,pad=0.3',
    ...
    ax.set_xticklabels([f'{s:.1f}%\n{labels_sorted[i]}'
                        for i, s in enumerate(sizes_sorted)], fontsize=13,
    ...
    # Add legend
    ax.legend(loc='lower left', fontsize=14, framealpha=0.9, frameon=True)
