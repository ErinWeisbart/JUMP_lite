...
echo "==================================================="
echo "Focused Sweep v7 — CP (54 configs) + DL (224 configs, split by batch_method)"
echo "==================================================="
echo ""
echo "Both sweeps output to variance_first_v7/ for unified summary."
echo "DL configs split into 4 method-specific sweeps to avoid Cartesian product overhead."
echo "  none:            16 configs  (2 norm x 2 fit_ctrl x 2 int x 2 pca)"
echo "  tvn_efaar:       80 configs  (above x 5 epsilon)"
echo "  tvn_original:    64 configs  (above x 4 k)"
echo "  spherize_global: 64 configs  (above x 4 epsilon)"
echo ""
...
# ============================================================
# PART 1: CellProfiler Models — focused_cp_v7 (54 configs each)
# ============================================================
...
echo "==================================================="
echo "PART 1: CellProfiler Models (${#CP_FEATURE_FILES[@]} datasets x 54 configs)"
echo "==================================================="
echo ""
...
for i in "${!CP_FEATURE_FILES[@]}"; do
    ...
    pixi run python -m norm_3.pipeline --multirun +sweep=focused_cp_v7 input.path="${feature_file}" hydra/launcher=joblib hydra.launcher.n_jobs=32 || {
...
# ============================================================
# PART 2: Deep Learning Models — 4 method-specific v7 configs (224 total per dataset)
# ============================================================
...
for i in "${!DL_FEATURE_FILES[@]}"; do
    ...
    # Run each method-specific config sequentially with GPU cleanup between sub-sweeps
    for sweep_cfg in "${DL_V7_CONFIGS[@]}"; do
        echo "  --- ${sweep_cfg} ---"
        pixi run python -m norm_3.pipeline --multirun +sweep=${sweep_cfg} input.path="${feature_file}" hydra/launcher=joblib hydra.launcher.n_jobs=16 || {
            echo "  Warning: ${sweep_cfg} encountered errors (some configs may have failed)"
        }
        cd ../..
        cleanup_gpu
        cd src/norm_3
    ...
    echo ""
    echo "Sweep complete for ${compression} (all 4 method configs)"
    echo ""
done

echo "==================================================="
echo "All Focused v7 Sweeps Complete!"
echo "==================================================="
echo ""
echo "Total: ${#CP_FEATURE_FILES[@]} CP datasets (54 configs) + ${#DL_FEATURE_FILES[@]} DL datasets (224 configs)"
echo ""

# Count results
echo "Final counts:"
if [ -d "src/norm_3/data/features/variance_first_v7" ]; then
    cd src/norm_3/data/features/variance_first_v7
...
echo "  cd src/norm_3 && pixi run python gather_sweep_results.py --sweep-dir data/features/variance_first_v7 --plot --filter-degenerate"
